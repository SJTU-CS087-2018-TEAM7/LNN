...
class LogicSyn(Dataset):
    ...
    data_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'LogicSyn')
    ...
    fields = meta["field_sizes"]
    num_fields = len(fields)
    max_length = num_fields
    num_features = sum(fields)
    feat_names = ["f" + str(i) for i in range(num_fields)]
    feat_sizes = [v for v in fields]
    ...
    def __init__(self, initialized=True):
        ...
        if not self.initialized:
            ...
            self.train_num_of_parts = self.raw_to_feature(raw_file='train.svm',
            ...
            self.test_num_of_parts = self.raw_to_feature(raw_file='test.svm',
